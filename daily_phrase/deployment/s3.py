...
class S3VideoManager:
    ...
    def download_video_clip(
        self, native_language: str, foreign_language: str, video_path: Path
    ) -> str:
        video_object_key = self._find_unpublished_video(
        ...
        self._s3_resource.Bucket(BUCKET_NAME).download_file(
            video_object_key, str(video_path / video_object_key)
        )
    ...
    def _get_object_tags(self, object_key: str):
        response = self._s3_client.get_object_tagging(
    ...
    def _find_unpublished_video(self, native_language, foreign_language):
        ...
        for obj in bucket.objects.all():
            ...
            if (
                tag_data.get("published") != "true"
                and tag_data.get("native_language") == native_language
                and tag_data.get("foreign_language") == foreign_language
            ):
                return obj.key
        return None
